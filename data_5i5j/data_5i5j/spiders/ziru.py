...
class ZiruSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 51):
            url = 'http://www.ziroom.com/z/nl/z2-d23008611.html?p=%s' % i
    ...
    def parse(self, response):
        ...
        img = open('1.png', 'rb')
        APP_ID = '15591114'
        API_KEY = '9eXOfNwMhxwKliBGUD45p6im'
        SECRET_KEY = 'uGjG0xp7llKKrUnpIWvh8D2HQcbLwhZ1 '
    ...
    def detail_data(self, response):
        ...
        item['types'] = types
        floor = response.xpath('.//ul[@class="detail_room"]/li[4]/text()').extract_first()
        floor = ''.join([i for i in floor][3:])
        item['floor'] = floor
        sub = response.xpath('.//ul[@class="detail_room"]/li[5]/span[1]/text()').extract_first()
        sub = ''.join([re.sub('\s.*', '', i) for i in sub.strip()][4:-1])
        sub = ''.join([re.sub('[\u4e00-\u9fa5].*', '', i) for i in sub])
        item['sub'] = sub
